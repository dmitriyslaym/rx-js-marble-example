...
const buy$ = fromEvent(
  ...
  mergeMap(
    (suggestedAssetIdToBuy) => iif(
      () => !!suggestedAssetIdToBuy,
      assets$.pipe(
        map(assetsMap => 
          assetsMap[suggestedAssetIdToBuy]
        ),
        filter(asset => asset),
...
const assets$ = timer(0, 5000)
  .pipe(
    tap((index) => console.log('interval', index)),
    mergeMap(() => assetsData$),
    scan((acc, asset) => {
      return { ...acc, [asset.id]: asset }
    }, {}),
...
`,
  ...
  'assetsAlive$': {
    name: 'assetsAlive$',
    code: `const { timer, fromEvent, iif, empty, range, from, ReplaySubject } = Rx;
const { reduce, multicast, refCount, mergeMap, switchMap, tap, scan, filter, map, takeWhile, debounceTime, distinctUntilChanged, startWith, pairwise, takeUntil } = RxOperators;
